...
try {
  ...
  await execSql(withUser(`
    select public.create_bank_loan_v1(
      ...
      '2026-01-01', '2026-01-01', 50000, 40000, 24, 1200, 'fixed', 'monthly', null, '2026-02-01',
  ...
  await execSql(withUser(`
    select public.create_bank_loan_v1(
      ...
      '2026-01-01', '2026-01-01', 10000, 10000, 12, 900, 'fixed', 'monthly', null, '2026-02-01',
  ...
  await execSql(withUser(`
    select public.record_debt_installment_advance_v1(
      ...
      810, 340, 0, 50, 0, true, '[]'::jsonb
    );
  `));
  const advanceRow = await execSql(`select event_type from public.debt_events where id = '${ids.advanceEvent}';`);
  if (advanceRow !== "installment_advance") {
    throw new Error(`Expected installment_advance event, got: ${advanceRow}`);
  }
  ...
  console.log("9. J, K, L, M: Testing Security, RLS & Cross-household Isolation...");
  ...
  console.log("SUCCESS! ALL 13 SQL SMOKE REQUIREMENTS (A-M) PASSED CLEANLY.");
} catch (err) {
