...
import type { Debt, DebtEvent, DebtEventInstallmentAllocation, DebtFinancingContract, DebtInstallment, DebtInstallmentCarriedAllocation, DebtKind, DebtScheduleVersion, FinancialAccount } from "../types";
...
export function DebtRefinanceForm({ debt, currentPrincipal, accounts, debtEvents = [], scheduleVersions = [], installments = [], allocations = [], carriedAllocations = [], debtFinancingContract = null, canWriteDebt, onSaved, onCancel, setToast }: DebtRefinanceFormProps) {
  ...
  const loadTargetSchedule = () => {
    try {
      ...
      if (!review.scheduleSource) throw new Error("El JSON V2 debe indicar scheduleSource para guardar el nuevo cronograma.");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const targetContract = {
        contractAuthority: targetScheduleSource === "contractual" ? "contractual" : "user_reported",
