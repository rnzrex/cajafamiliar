...
describe("DEBT-2C Exhaustive Hardened Invariants (32 tests)", () => {
  ...
  it("4c. translates non-current bank schedule errors exactly", () => {
    expect(translateDebtError(new Error("DEBT_PREPAYMENT_SCHEDULE_NOT_CURRENT"))).toBe("El cronograma del banco no coincide con el principal actual del crédito. Carga una versión vigente que incorpore los pagos posteriores.");
  });
