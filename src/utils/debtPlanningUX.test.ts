...
describe("DEBT-3B UX & Planning Helpers", () => {
  ...
  it("9. unknown amount shows semantics without false-zero", () => {
    const sv = scheduleVersion();
    const iKnown = installment({ id: "ik", dueDate: "2026-08-10", expectedAmount: 1000 });
    const iUnknown = installment({ id: "iu", dueDate: "2026-08-15", expectedAmount: null, installmentNumber: 2 });
  ...
  it("13. dashboard debt attention sorted by daysUntilDue ascending", () => {
    const sv = scheduleVersion();
    const iToday = installment({ id: "itd", dueDate: "2026-08-21" });
  ...
  it("15. multi-currency handling (currencyCode present and summarized by currency)", () => {
    ...
    expect(summary.byCurrency["USD"].pendingKnownAmount).toBe(300);
  });
});
